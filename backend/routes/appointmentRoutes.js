...
router.post("/", protect, student, async (req, res) => {
  try {
    ...
    if (existingAppointment) {
      return res
        .status(400)
        .json({ message: "Timeslot is already booked or pending" });
    }
  ...
  try {
    if (req.user.role === "student") {
      ...
      const appointments = await Appointment.find({
        counselorId: counselorProfile._id,
      })
...
router.put("/:id/status", protect, counselor, async (req, res) => {
  try {
    ...
    if (appointment) {
      ...
      if (
        req.user.role !== "admin" &&
        String(appointment.counselorId) !== String(counselorProfile._id)
      ) {
        return res
          .status(401)
    ...
    if (appointment) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({
            message: "Cannot cancel a completed or rejected appointment",
          });
      }
